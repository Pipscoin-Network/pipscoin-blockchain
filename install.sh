...
# Check current Pipscoin SSL version to prevent update on old SSL
if [ -e ../.pipscoin/mainnet/config/ssl/ca/pipscoin_ca.crt ]; then
	PIPSCOIN_SSL_SERIAL=$(openssl x509 -noout -in ../.pipscoin/mainnet/config/ssl/ca/pipscoin_ca.crt -serial)
	if [ $PIPSCOIN_SSL_SERIAL = "serial=5C8A71239328650EB9FEF85CEC32BF779CA6A0C5" ]; then
		echo ""
		echo "WARNING:"
		echo "Old version of Pipscoin Blockchain SSL has been detected."
		echo "Please visit https://pipscoin.net/sslupdate/ for further instructions."
		echo ""
...
echo ""
echo "Visit our Website to learn more about Pipscoin:"
echo "https://pipscoin.net"
echo ""
