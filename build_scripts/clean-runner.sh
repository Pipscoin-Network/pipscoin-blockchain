...
rm -rf ../venv || true
rm -rf venv || true
rm -rf pipscoin_blockchain.egg-info || true
rm -rf build_scripts/final_installer || true
rm -rf build_scripts/dist || true
rm -rf build_scripts/pyinstaller || true
rm -rf pipscoin-blockchain-gui/build || true
rm -rf pipscoin-blockchain-gui/daemon || true
rm -rf pipscoin-blockchain-gui/node_modules || true
rm pipscoin-blockchain-gui/temp.json || true
( cd "$PWD/pipscoin-blockchain-gui" && git checkout HEAD -- package-lock.json ) || true
cd "$PWD" || true
...
npm uninstall -g @pipscoin-network/notarize-cli || true
