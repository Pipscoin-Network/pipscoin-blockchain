...
import { useCreateOfferForIdsMutation } from '@pipscoin/api-react';
import {
  Button,
  Divider,
  Grid,
...
import { pipscoinToMojo, catToMojo } from '@pipscoin/core';
import fs from 'fs';
  ...
  function updateOffer(offer: { [key: string]: number | string }, row: OfferEditorRowData, debit: boolean) {
    ...
    if (assetWalletId > 0) {
      ...
      if (walletType === WalletType.STANDARD_WALLET) {
        mojoAmount = Number.parseFloat(pipscoinToMojo(amount));
      }
