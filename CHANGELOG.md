...
## 1.2.9 Pipscoin blockchain 2021-10-01

### Changed

- Changed "About" section in client to indicate correct release version.

## 1.2.8 Pipscoin blockchain 2021-09-30
...
- Added support for DID, our decentralized identity solution, as a building block toward Pipscoin's broader set of DID capabilities.
...
- Beta builds are built on every merge to main, and are now available from <https://pipscoin.net/download/>.
...
- We have made many full node changes to support our upcoming Pipscoin Asset Token (CAT) standard and our upcoming standalone light wallet, which will use Pipscoin's new electrum-style protocol to enable faster wallet syncing.
...
- Several clvm_rs updates to support our upcoming Pipscoin Asset Token (CAT) standard.
...
- Added a py.typed file that allows other projects that pip install pipscoin-blockchain to type check using our functions with mypy.
- Added an RPC for coin records by multiple coin names.
- Enabled querying AAAA records for DNS Introducer.
- We now set the version for the GUI when doing a manual install using the install-gui.sh script. Uses a python helper to get the version of the pipscoin install and then converts it into proper npm format and puts that into package.json.
...
- Thanks @altendky for changing the default to paginate to pipscoin wallet get_transactions to address cases such as piping and output redirection to a file where the command previously just hung while waiting for the user to press c for the next page.
...
- Switched IP address lookup to first use Pipscoin's service ip.pipscoin.net.
...
## 1.2.3 Pipscoin blockchain 2021-07-26
...
- Thanks @mishan for your change to 'pipscoin plotnft show' to display Percent Successful Points.
...
## 1.2.2 Pipscoin blockchain 2021-07-13

### Fixed

- Converted test_rom.py to use pytest and fixed test_singleton.
- Thanks to @yshklarov for help fixing [#7273](https://github.com/Pipscoin-Network/pipscoin-blockchain/issues/7273), which bundled CA store to support pools for some farming systems, including M1 Apple computers. This enables those machines to properly connect to pools, and fixes the issue.

## 1.2.1 Pipscoin blockchain 2021-07-12
...
- Generators have been re-worked in CLVM. We added a chialisp deserialization puzzle and improved the low-level generator. We reduce the accepted atom size to 1MB during PipscoinLisp native deserialization.
...
- Pipscoin wheels not on Pypi and some dependecies not found there also are now on pypi.pipscoin.net.
...
- 'pipscoin wallet send' now takes in TPIPS or PIPS as units instead of bytes.
...
- clvm bytecode is now generated and confirmed that the checked-in clvm and PipscoinLisp code matches the CI compiled code.
