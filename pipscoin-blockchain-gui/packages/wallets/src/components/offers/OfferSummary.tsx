...
import { type OfferSummaryRecord  } from '@pipscoin/api';
import {
  Flex,
  StateColor,
  mojoToPipscoin,
...
export default function OfferSummary(props: Props) {
  ...
  const makerAmount = makerEntries[0][0].toLowerCase() === 'pips' ? Number(mojoToPipscoin(makerEntries[0][1])) : Number(mojoToCAT(makerEntries[0][1]));
  const takerAmount = takerEntries[0][0].toLowerCase() === 'pips' ? Number(mojoToPipscoin(takerEntries[0][1])) : Number(mojoToCAT(takerEntries[0][1]));
