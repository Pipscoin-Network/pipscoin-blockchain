...
cp -r dist/daemon ../pipscoin-blockchain-gui/packages/gui
cd .. || exit
...
# sets the version for pipscoin-blockchain in package.json
cp package.json package.json.orig
jq --arg VER "$PIPSCOIN_INSTALLER_VERSION" '.version=$VER' package.json > temp.json && mv temp.json package.json
...
--appVersion=$PIPSCOIN_INSTALLER_VERSION --executable-name=pipscoin-blockchain
LAST_EXIT_CODE=$?
...
if [ "$REDHAT_PLATFORM" = "x86_64" ]; then
	...
	# Disables build links from the generated rpm so that we dont conflict with other packages. See https://github.com/Pipscoin-Network/pipscoin-blockchain/issues/3846
  ...
  --license ../LICENSE --options.bin pipscoin-blockchain --options.name pipscoin-blockchain
  LAST_EXIT_CODE=$?
  if [ "$LAST_EXIT_CODE" -ne 0 ]; then
