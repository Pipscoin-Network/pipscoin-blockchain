...
from pipscoin.daemon.keychain_proxy import KeychainProxy, connect_to_keychain_and_validate, wrap_local_keychain
from pipscoin.plotting.util import add_plot_directory
from pipscoin.plotting.util import stream_plot_info_ph, stream_plot_info_pk
...
class PlotKeysResolver:
    ...
    async def get_farmer_public_key(self, keychain_proxy: Optional[KeychainProxy] = None) -> G1Element:
        ...
        if sk is None:
            raise RuntimeError(
                "No keys, please run 'pipscoin keys add', 'pipscoin keys generate' or provide a public key with -f"
            )
    ...
    async def get_pool_public_key(self, keychain_proxy: Optional[KeychainProxy] = None) -> G1Element:
        ...
        if sk is None:
            raise RuntimeError(
                "No keys, please run 'pipscoin keys add', 'pipscoin keys generate' or provide a public key with -p"
            )
