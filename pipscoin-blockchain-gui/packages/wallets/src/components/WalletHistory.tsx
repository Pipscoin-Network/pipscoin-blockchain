...
import { Card, CardKeyValue, CopyToClipboard, Flex, Loading, StateColor, TableControlled, toBech32m, useCurrencyCode, mojoToPipscoinLocaleString, mojoToCATLocaleString } from '@pipscoin/core';
import { useGetOfferRecordMutation, useGetSyncStatusQuery } from '@pipscoin/api-react';
import styled from 'styled-components';
import type { Row } from '@pipscoin/core';
import { WalletType, TransactionType } from '@pipscoin/api';
...
const getCols = (type: WalletType, isSyncing, getOfferRecord, navigate) => [
        ...
        <>
          ...
          <strong>
            {type === WalletType.CAT
              ...
              : mojoToPipscoinLocaleString(row.amount)}
          </strong>
          &nbsp;
          {metadata.unit}
        </>
  ...
  },
  {
    field: (row: Row, metadata) => (
      <>
        <strong>{mojoToPipscoinLocaleString(row.feeAmount)}</strong>
        &nbsp;
        {metadata.feeUnit}
      </>
    ),
