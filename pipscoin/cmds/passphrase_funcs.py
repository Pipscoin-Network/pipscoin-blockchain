...
from pipscoin.daemon.client import acquire_connection_to_daemon
from pipscoin.util.keychain import Keychain, obtain_current_passphrase, supports_os_passphrase_storage
from pipscoin.util.keyring_wrapper import DEFAULT_PASSPHRASE_IF_NO_MASTER_PASSPHRASE
from pipscoin.util.misc import prompt_yes_no
from pipscoin.util.ws_message import WsRpcMessage
    ...
    try:
        if supports_os_passphrase_storage():
            ...
            print(
                ...
                "Other Pipscoin processes will be able to access your keys without prompting for your passphrase."
            )
...
def read_passphrase_from_file(passphrase_file: TextIOWrapper) -> str:
    passphrase = passphrase_file.read().rstrip(os.environ.get("PIPSCOIN_PASSPHRASE_STRIP_TRAILING_CHARS", "\r\n"))
...
def initialize_passphrase() -> None:
    if Keychain.has_master_passphrase():
        ...
        print("\nUse 'pipscoin passphrase set' or 'pipscoin passphrase remove' to update or remove your passphrase")
        sys.exit(1)
