...
import { OfferTradeRecord } from '@pipscoin/api';
import fs from 'fs';
...
import { Trade as TradeIcon } from '@pipscoin/icons';
import { useCancelOfferMutation, useGetOfferDataMutation, useGetWalletsQuery } from '@pipscoin/api-react';
...
function ConfirmOfferCancellation(props: ConfirmOfferCancellationProps) {
  ...
  async function handleConfirm() {
    const { fee: pipsFee } = methods.getValues();
    var fee = 0;

    if (cancelWithTransaction) {
      fee = Number.parseFloat(pipscoinToMojo(pipsFee));
    }
      ...
      <Grid container>
        <Grid xs={12} md={6} lg={5} item>
          <CardHero>
            ...
            <Typography variant="body1">
              <Trans>
                Create an offer to exchange PIPS or other tokens. View an offer to inspect and accept an offer made by another party.
              </Trans>
            </Typography>
