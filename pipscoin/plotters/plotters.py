...
from pipscoin.plotters.bladebit import get_bladebit_install_info, plot_bladebit
from pipscoin.plotters.chiapos import get_chiapos_install_info, plot_pipscoin
from pipscoin.plotters.madmax import get_madmax_install_info, plot_madmax
from pipscoin.plotters.install_plotter import install_plotter
...
def call_plotters(root_path: Path, args):
    # Add `plotters` section in PIPSCOIN_ROOT.
    pipscoin_root_path = root_path
    ...
    if not os.path.exists(root_path):
        print(f"Creating plotters folder within PIPSCOIN_ROOT: {root_path}")
        try:
            os.mkdir(root_path)
        except Exception as e:
    ...
    build_parser(subparsers, root_path, pipscoin_plotter, "chiapos", "Pipscoinpos Plotter")
    ...
    if args.plotter == "chiapos":
        plot_pipscoin(args, pipscoin_root_path)
    if args.plotter == "madmax":
        plot_madmax(args, pipscoin_root_path, root_path)
    if args.plotter == "bladebit":
        plot_bladebit(args, pipscoin_root_path, root_path)
