...
from pipscoin.cmds.passphrase_funcs import get_current_passphrase
from pipscoin.daemon.client import DaemonProxy, connect_to_daemon_and_validate
from pipscoin.util.keychain import Keychain, KeyringMaxUnlockAttempts
...
def launch_start_daemon(root_path: Path) -> subprocess.Popen:
    ...
    pipscoin = sys.argv[0]
    process = subprocess.Popen(f"{pipscoin} run_daemon --wait-for-unlock".split(), stdout=subprocess.PIPE)
    return process
