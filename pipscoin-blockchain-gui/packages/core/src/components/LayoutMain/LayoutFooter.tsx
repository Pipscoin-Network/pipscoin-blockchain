...
async function openFAQURL(): Promise<void> {
  try {
    ...
    await shell.openExternal('https://github.com/Pipscoin-Network/pipscoin-blockchain/wiki/FAQ');
  }
  catch (e) {
    console.error(e);
  }
...
async function openSendFeedbackURL(): Promise<void> {
  try {
    const shell: Shell = (window as any).shell;
    await shell.openExternal('https://feedback.pipscoinnetwork.org/lightwallet');
  }
  catch (e) {
    console.error(e);
  }
