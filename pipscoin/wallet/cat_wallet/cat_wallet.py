...
from pipscoin.wallet.cat_wallet.cat_info import CATInfo, LegacyCATInfo
...
from pipscoin.wallet.cat_wallet.lineage_store import CATLineageStore
from pipscoin.wallet.lineage_proof import LineageProof
from pipscoin.wallet.payment import Payment
from pipscoin.wallet.puzzles.genesis_checkers import ALL_LIMITATIONS_PROGRAMS
...
class CATWallet:
    ...
    async def create_new_cat_wallet(
        ...
        try:
            pipscoin_tx, spend_bundle = await ALL_LIMITATIONS_PROGRAMS[
        ...
        pipscoin_tx = dataclasses.replace(pipscoin_tx, spend_bundle=spend_bundle)
        await self.standard_wallet.push_transaction(pipscoin_tx)
    ...
    async def create_tandem_pips_tx(
        self,
        fee: uint64,
        ...
        if fee > amount_to_claim:
            pipscoin_coins = await self.standard_wallet.select_coins(fee)
            origin_id = list(pipscoin_coins)[0].name()
            pipscoin_tx = await self.standard_wallet.generate_signed_transaction(
                uint64(0),
                (await self.standard_wallet.get_new_puzzlehash()),
                fee=uint64(fee - amount_to_claim),
                coins=pipscoin_coins,
                origin_id=origin_id,  # We specify this so that we know the coin that is making the announcement
                negative_change_allowed=False,
                coin_announcements_to_consume={announcement_to_assert} if announcement_to_assert is not None else None,
            )
            assert pipscoin_tx.spend_bundle is not None

            message = None
            for spend in pipscoin_tx.spend_bundle.coin_spends:
            ...
            pipscoin_coins = await self.standard_wallet.select_coins(fee)
            selected_amount = sum([c.amount for c in pipscoin_coins])
            pipscoin_tx = await self.standard_wallet.generate_signed_transaction(
        ...
        return pipscoin_tx, announcement
    ...
    async def generate_unsigned_spendbundle(
        ...
        # Figure out if we need to absorb/melt some PIPS as part of this
        regular_pipscoin_to_claim: int = 0
        if payment_amount > starting_amount:
            fee = uint64(fee + payment_amount - starting_amount)
        elif payment_amount < starting_amount:
            regular_pipscoin_to_claim = payment_amount

        need_pipscoin_transaction = (fee > 0 or regular_pipscoin_to_claim > 0) and (fee - regular_pipscoin_to_claim != 0)
        ...
        for coin in cat_coins:
            if first:
                ...
                if need_pipscoin_transaction:
                    if fee > regular_pipscoin_to_claim:
                        pipscoin_tx, _ = await self.create_tandem_pips_tx(
                            fee, uint64(regular_pipscoin_to_claim), announcement_to_assert=announcement
                        )
                    ...
                    elif regular_pipscoin_to_claim > fee:
                        pipscoin_tx, _ = await self.create_tandem_pips_tx(fee, uint64(regular_pipscoin_to_claim))
                        innersol = self.standard_wallet.make_solution(
                            primaries=primaries,
                            coin_announcements={announcement.message},
        ...
        pipscoin_spend_bundle = SpendBundle([], G2Element())
        if pipscoin_tx is not None and pipscoin_tx.spend_bundle is not None:
            pipscoin_spend_bundle = pipscoin_tx.spend_bundle

        return (
            SpendBundle.aggregate(
                [
                    cat_spend_bundle,
                    pipscoin_spend_bundle,
                ]
            ),
            pipscoin_tx,
        )
    ...
    async def generate_signed_transaction(
        ...
        unsigned_spend_bundle, pipscoin_tx = await self.generate_unsigned_spendbundle(
            payments,
            fee,
            coins=coins,
        ...
        if pipscoin_tx is not None:
            tx_list.append(
                TransactionRecord(
                    confirmed_at_height=pipscoin_tx.confirmed_at_height,
                    created_at_time=pipscoin_tx.created_at_time,
                    to_puzzle_hash=pipscoin_tx.to_puzzle_hash,
                    amount=pipscoin_tx.amount,
                    fee_amount=pipscoin_tx.fee_amount,
                    confirmed=pipscoin_tx.confirmed,
                    sent=pipscoin_tx.sent,
                    spend_bundle=None,
                    additions=pipscoin_tx.additions,
                    removals=pipscoin_tx.removals,
                    wallet_id=pipscoin_tx.wallet_id,
                    sent_to=pipscoin_tx.sent_to,
                    trade_id=pipscoin_tx.trade_id,
                    type=pipscoin_tx.type,
                    name=pipscoin_tx.name,
                    memos=[],
                )
            )
