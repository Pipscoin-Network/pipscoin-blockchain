...
from pipscoin.cmds.init_funcs import check_keys, pipscoin_init
from pipscoin.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from pipscoin.daemon.keychain_server import KeychainServer, keychain_commands
from pipscoin.daemon.windows_signal import kill
from pipscoin.plotters.plotters import get_available_plotters
from pipscoin.plotting.util import add_plot_directory
...
service_plotter = "pipscoin_plotter"
...
if getattr(sys, "frozen", False):
    name_map = {
        ...
        "pipscoin_seeder": "start_seeder",
        "pipscoin_crawler": "start_crawler",
    }
...
class WebSocketServer:
        ...
        if self.ssl_context.minimum_version is not ssl.TLSVersion.TLSv1_3:
            self.log.warning(
                (
                    ...
                    "A future version of Pipscoin will require TLS1.3."
                ),
                ssl.OPENSSL_VERSION,
            )
        else:
        ...
        try:
            if Keychain.master_passphrase_is_valid(key, force_reload=True):
                ...
                try:
                    ...
                    self.log.exception("Failed to migrate keys silently. Run `pipscoin keys migrate` manually.")
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        ...
        command_args: List[str] = ["pipscoin", "plotters", plotter]
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    if service_command == "pipscoin_full_node_simulator":
        # Set the -D/--connect_to_daemon flag to signify that the child should connect
        # to the daemon to access the keychain
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in pipscoin_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    pipscoin_init(root_path, should_check_keys=(not wait_for_unlock))
