...
from pipscoin.pools.pool_config import PoolWalletConfig, load_pool_config, add_auth_key
...
from pipscoin.util.config import load_config, save_config, config_path_for_filename, get_config_lock
...
class Farmer:
    ...
    async def setup_keys(self) -> bool:
        no_keys_error_str = "No keys exist. Please run 'pipscoin keys generate' or open the UI."
        ...
        if "pips_target_address" not in self.config:
            self.config = config["farmer"]
        if "pips_target_address" not in self.pool_config:
            self.pool_config = config["pool"]
        if "pips_target_address" not in self.config or "pips_target_address" not in self.pool_config:
            log.debug("pips_target_address missing in the config")
            return False
        ...
        self.pool_target_encoded = self.pool_config["pips_target_address"]
