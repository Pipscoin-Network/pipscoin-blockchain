...
import { useGetWalletBalanceQuery } from '@pipscoin/api-react';
import { TransactionType } from '@pipscoin/api';
import type { Transaction } from '@pipscoin/api';
import { mojoToPipscoin, blockHeightToTimestamp } from '@pipscoin/core';
  ...
  const points = [
    {
      ...
      y: Math.max(0, Number(mojoToPipscoin(start))),
      tooltip: mojoToPipscoin(balance),
    },
  ];
