...
import { /* byte_to_pipscoin_string, */ pipscoin_to_byte } from '../../../util/pipscoin';
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {byte_to_pipscoin_string(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
...
function SendCard(props: SendCardProps) {
  ...
  function handleSubmit(data: SendTransactionData) {
    ...
    const amountValue = Number.parseFloat(pipscoin_to_byte(amount));
    const feeValue = Number.parseFloat(pipscoin_to_byte(fee));
