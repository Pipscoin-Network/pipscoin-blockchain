...
from pipscoin.clvm.spend_sim import SpendSim, SimClient
...
from pipscoin.wallet.cat_wallet.cat_utils import (
    CAT_MOD,
    construct_cat_puzzle,
    SpendableCAT,
    unsigned_spend_bundle_for_spendable_cats,
)
from pipscoin.wallet.payment import Payment
from pipscoin.wallet.trading.offer import Offer, NotarizedPayment
...
class TestOfferLifecycle:
    ...
    # This method takes a dictionary of strings mapping to amounts and generates the appropriate CAT/PIPS coins
    ...
    async def test_complex_offer(self, setup_sim):
        ...
        try:
            ...
            pipscoin_coins: List[Coin] = all_coins[None]
            red_coins: List[Coin] = all_coins["red"]
            blue_coins: List[Coin] = all_coins["blue"]

            # Create an PIPS Offer for RED
            pipscoin_requested_payments: Dict[Optional[bytes32], List[Payment]] = {
            ...
            pipscoin_requested_payments: Dict[Optional[bytes32], List[NotarizedPayment]] = Offer.notarize_payments(
                pipscoin_requested_payments, pipscoin_coins
            )
            pipscoin_announcements: List[Announcement] = Offer.calculate_announcements(pipscoin_requested_payments)
            pipscoin_secured_bundle: SpendBundle = self.generate_secure_bundle(pipscoin_coins, pipscoin_announcements, 1000)
            pipscoin_offer = Offer(pipscoin_requested_payments, pipscoin_secured_bundle)
            assert not pipscoin_offer.is_valid()

            # Create a RED Offer for PIPS
            ...
            new_offer = Offer.aggregate([pipscoin_offer, red_offer])
            ...
            # Create yet another offer of BLUE for PIPS and RED
            ...
            assert new_offer.summary() == (
                ...
                {"pips": 900, str_to_tail_hash("red").hex(): 350},
            )
