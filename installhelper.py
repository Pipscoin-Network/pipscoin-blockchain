...
# Gets the version from the result of "pipscoin version"
...
def get_pipscoin_version() -> str:
    version: str = "0.0"
    output = subprocess.run(["pipscoin", "version"], capture_output=True)
...
def update_version(package_json_path: str):
    ...
    data["version"] = get_pipscoin_version()
...
if __name__ == "__main__":
    update_version(f"{os.path.dirname(__file__)}/pipscoin-blockchain-gui/package.json")
    update_version(f"{os.path.dirname(__file__)}/pipscoin-blockchain-gui/packages/gui/package.json")
