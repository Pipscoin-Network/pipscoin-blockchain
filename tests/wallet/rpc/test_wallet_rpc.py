...
from pipscoin.wallet.transaction_sorting import SortKey
from pipscoin.wallet.util.compute_memos import compute_memos
...
class TestWalletRpc:
    ...
    async def test_wallet_rpc(self, two_wallet_nodes, trusted):
        ...
        try:
            ...
            # Test RELEVANCE
            await client.send_transaction("1", 1, encode_puzzle_hash(ph_2, "pips"))  # Create a pending tx
            ...
            await client.send_transaction("1", 1, encode_puzzle_hash(ph_by_addr, "pips"))
            await client.farm_block(encode_puzzle_hash(ph_by_addr, "pips"))
            ...
            tx_for_address = await wallet_rpc_api.get_transactions(
                {"wallet_id": "1", "to_address": encode_puzzle_hash(ph_by_addr, "pips")}
            )
            ...
            # Create an offer of 5 pipscoin for one CAT
            ...
            assert summary == {"offered": {"pips": 5}, "requested": {col.hex(): 1}, "fees": 1}
