...
import { useGetBlockQuery, useGetBlockRecordQuery  } from '@pipscoin/api-react'
...
export default function Block() {
  ...
  const poolReward = mojoToPipscoin(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojoToPipscoin(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const pipscoinFees = blockRecord.fees !== undefined
    ? mojoToPipscoin(blockRecord.fees)
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.pipscoinexplorer.org/blockchain/puzzlehash/${blockRecord.farmerPuzzleHash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.pipscoinexplorer.org/blockchain/puzzlehash/${blockRecord.poolPuzzleHash}`}
        >
          {currencyCode
            ? toBech32m(
